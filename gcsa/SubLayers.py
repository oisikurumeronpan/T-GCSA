...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q_r, q_i, k_r, k_i, v_r, v_i, dwm):
        ...
        q_r = self.dropout(self.frc(q_r))
        q_i = self.dropout(self.fic(q_i))
        q_r += residual_r
        q_i += residual_i

        q_r = self.layer_norm(q_r)
        q_i = self.layer_norm(q_i)
...
class PositionwiseFeedForward(nn.Module):
    ...
    def forward(self, x_r, x_i):
        ...
        x_r2 = self.dropout(self.w_2r(x_r1) - self.w_2i(x_i1))
        x_i2 = self.dropout(self.w_2i(x_r1) + self.w_2r(x_i1))
        x_r2 += x_r
        x_i2 += x_i

        x_r2 = self.layer_norm(x_r2)
        x_i2 = self.layer_norm(x_i2)
